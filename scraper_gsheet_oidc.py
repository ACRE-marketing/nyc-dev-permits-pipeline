...
# -*- coding: utf-8 -*-
"""
NYC 开发&许可 → Google Sheet（无密钥 OIDC 方式，适配 GitHub Actions Workload Identity Federation）
依赖：requests beautifulsoup4 feedparser python-dateutil pandas gspread google-auth google-auth-oauthlib google-auth-httplib2
环境变量：
  - GSHEET_ID （必需）
  - GSHEET_TAB （可选，默认 'Daily'）
  - LOOKBACK_HOURS （可选，默认 24）
  - NYC_SODA_APP_TOKEN （可选）
认证说明：
  - 不需要 Service Account key。工作流里使用 google-github-actions/auth@v2 完成 OIDC 登录，
    然后本脚本通过 google.auth.default() 获取短期凭证并写入 Google Sheets。
"""
...
def open_gsheet(spreadsheet_id: str, scopes=None):
    if scopes is None:
        scopes = ["https://www.googleapis.com/auth/spreadsheets"]
    # 使用 OIDC 导出的 ADC（由 google-github-actions/auth 提供）
...
def append_history_to_sheet(df: pd.DataFrame, spreadsheet_id: str, worksheet_name="Daily"):
    ...
    if existing and existing[0] == header:
        ...
        ws.clear(); ws.update([header]); data_rows = []
    existing_keys = set()
